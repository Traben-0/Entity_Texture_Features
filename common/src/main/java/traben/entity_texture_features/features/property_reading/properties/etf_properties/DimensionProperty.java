...
public class DimensionProperty extends StringArrayOrRegexProperty {
    ...
    public @Nullable String getValueFromEntity(ETFEntity etfEntity) {
        ...
        DimensionType dimKey = etfEntity.etf$getWorld().getDimension();
        if (dimKey == null) return null;

        Identifier key = dimKey.getEffects();
        if (key == null) return null;

        String output;
        if (key.equals(DimensionType.OVERWORLD_ID) || key.getPath().equals("overworld_caves")) {
            output = "overworld";
        } else if (key.equals(DimensionType.THE_NETHER_ID)) {
            output = "the_nether";
        } else if (key.equals(DimensionType.THE_END_ID)) {
            output = "the_end";
        } else {
            //modded
            output = key.toString();
