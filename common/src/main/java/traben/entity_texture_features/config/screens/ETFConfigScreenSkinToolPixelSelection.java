    ...
    protected void init() {
        ...
        for (int x = MODE.startX; x < MODE.startX + 8; x++) {
            for (int y = MODE.startY; y < MODE.startY + 8; y++) {
                int color = etfParent.currentEditorSkin.getPixelColor(x, y);
                if (color != 0) {
        ...
        for (int x = 0; x < 64; x++) {
            for (int y = 0; y < 64; y++) {
                ...
                ButtonWidget butt = new ButtonWidget((int) ((this.width * 0.35) + (x * pixelSize)), (int) ((this.height * 0.2) + (y * pixelSize)), pixelSize, pixelSize,
                        Text.of(""),
                        (button) -> {
                            int colorAtPixel = etfParent.currentEditorSkin.getPixelColor(finalX, finalY);
        ...
        for (int x = MODE.startX; x < MODE.startX + 8; x++) {
            for (int y = MODE.startY; y < MODE.startY + 8; y++) {
                if (integerSet.isEmpty()) {
                    etfParent.currentEditorSkin.setPixelColor(x, y, 0);
                } else {
                    etfParent.currentEditorSkin.setPixelColor(x, y, integerSet.get(0));
                    integerSet.remove(0);
                }
            }
        }
...
//            drawTextWithShadow(matrices, textRenderer, ETFVersionDifferenceHandler.getTextFromTranslation("config." + MOD_ID + ".player_skin_editor.iris_message"), width / 8, (int) (this.height * 0.15), 0xFF5555);
    ...
    public void drawEntity(int x, int y, int size, float mouseX, float mouseY, LivingEntity entity) {
        ...
        RenderSystem.runAsFancy(() -> entityRenderDispatcher.render(entity, 0.0, 0.0, 0.0, 0.0F, 1.0F, matrixStack, immediate, 0x800080 /*15728880*/));
        immediate.draw();
