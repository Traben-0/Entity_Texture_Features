...
//                int blinkModifierBySystemTimeInTicks = (int) ((System.currentTimeMillis() / 50) % (30 + (ETFConfig.getInstance().blinkLength * 2)));
//                ETFManager.getInstance().ENTITY_BLINK_TIME.put(player.getUuid(), player.world.getTime() + blinkModifierBySystemTimeInTicks - (15 + ETFConfigData.blinkLength));
                ...
                drawTextWithShadow(matrices, textRenderer, Text.of("Player is null for some reason!"), width / 7, (int) (this.height * 0.4), 0xFFFFFF);
                drawTextWithShadow(matrices, textRenderer, Text.of("Cannot load player to render!"), width / 7, (int) (this.height * 0.45), 0xFFFFFF);
            }
        }
    ...
    private class ConfirmScreen extends Screen{
        ...
        protected void init() {
            ...
            addDrawableChild(ButtonWidget.builder(ScreenTexts.YES,(button)-> {
                allowOverrides = true;
                MinecraftClient.getInstance().setScreen(this.parent);
            }).dimensions(width/ 2 - 210, height / 2 + 50, 200, 20).build());

            addDrawableChild(ButtonWidget.builder(ScreenTexts.NO,(button)-> close()).dimensions(width/ 2 + 10, height / 2 + 50, 200, 20).build());
        }
        ...
        public void render(final MatrixStack matrices, final int mouseX, final int mouseY, final float delta) {
            ...
            drawCenteredTextWithShadow(matrices, textRenderer, ETFVersionDifferenceHandler.getTextFromTranslation("config.entity_texture_features.skin_editor.overlays.1"), width / 2, height / 2, 0xFFFFFF);
            drawCenteredTextWithShadow(matrices, textRenderer, ETFVersionDifferenceHandler.getTextFromTranslation("config.entity_texture_features.skin_editor.overlays.2"), width / 2, height / 2+11, 0xFFFFFF);
            drawCenteredTextWithShadow(matrices, textRenderer, ETFVersionDifferenceHandler.getTextFromTranslation("config.entity_texture_features.skin_editor.overlays.3"), width / 2, height / 2+22, 0xFFFFFF);
        }
    }
