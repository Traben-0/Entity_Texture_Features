...
//                createPatchedTextures();
    ...
    private boolean havePatchedBefore = false;

    /**
     * Patches the base textures against their emissive overlays,
     * this is the only way to get armor emissives rendering correctly
     */
    public void patchBaseTextures() {
        if(havePatchedBefore || isPatched_CurrentlyOnlyArmor())
            return;

        havePatchedBefore = true;
    ...
    public Identifier getTextureIdentifier(@Nullable ETFEntity entity, boolean forcePatchedTexture) {
        ...
        if (isPatched_CurrentlyOnlyArmor() && (forcePatchedTexture || (ETFConfigData.enableEmissiveTextures && ETFVersionDifferenceHandler.areShadersInUse()) || MinecraftClient.getInstance().currentScreen instanceof ETFConfigScreen)) {
            //patched required
