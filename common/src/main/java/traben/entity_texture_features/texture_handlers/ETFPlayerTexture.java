...
public class ETFPlayerTexture {
    ...
    private static int[] getSkinPixelBounds(String choiceKey) {
        switch (choiceKey) {
            ...
            case "cape5" : return  new int[]{44, 48, 51, 51};
    ...
    public void renderFeatures(MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int light, PlayerEntityModel<PlayerEntity> model) {
        if (canUseFeaturesForThisPlayer()) {
            ...
            if (hasVillagerNose) {
                ...
                customPlayerModel.textureNose.render(matrixStack, noseVertex, light, OverlayTexture.DEFAULT_UV, 1.0F, 1.0F, 1.0F, 1.0F);
                if (texturedNoseIdentifierEmissive != null) {
                    ...
                    customPlayerModel.textureNose.render(matrixStack, noseVertex_e, ETFClientCommon.MAX_LIGHT_COORDINATE, OverlayTexture.DEFAULT_UV, 1.0F, 1.0F, 1.0F, 1.0F);
                }
                if (texturedNoseIdentifierEnchanted != null) {
                    ...
                    customPlayerModel.textureNose.render(matrixStack, noseVertex_ench, ETFClientCommon.MAX_LIGHT_COORDINATE, OverlayTexture.DEFAULT_UV, 1.0F, 1.0F, 1.0F, 1.0F);
                }
            }
...
//    }
    ...
    public void checkTexture(boolean skipSkinLoad) {
        ...
        if (!skipSkinLoad) {
            try {
                PlayerSkinTexture skin = (PlayerSkinTexture) ((PlayerSkinProviderAccessor) MinecraftClient.getInstance().getSkinProvider()).getTextureManager().getTexture(normalVanillaSkinIdentifier);
                ...
                try {
                    ...
                    PlayerSkinTexture cape = (PlayerSkinTexture) ((PlayerSkinProviderAccessor) MinecraftClient.getInstance().getSkinProvider()).getTextureManager().getTexture(capeId);
                    if (cape != null) {
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                ...
                int[] choiceBoxChoices = {
                        getSkinPixelColourToNumber(originalSkin.getPixelColor(52, 16)),//blink
                        getSkinPixelColourToNumber(originalSkin.getPixelColor(52, 17)),//jacket choice
                        getSkinPixelColourToNumber(originalSkin.getPixelColor(52, 18)),//jacket height
                        getSkinPixelColourToNumber(originalSkin.getPixelColor(52, 19)),//blink height
                        getSkinPixelColourToNumber(originalSkin.getPixelColor(53, 16)),//cape choice
                        getSkinPixelColourToNumber(originalSkin.getPixelColor(53, 17)),//nose choice
                    ...
                    if (noseChoice == 1) {
                        ...
                        for (int x = bounds[0]; x <= bounds[2]; x++) {
                            ...
                            for (int y = bounds[1]; y <= bounds[3]; y++) {
                                noseTexture.setPixelColor(noseX, noseY, originalSkin.getPixelColor(x, y));
                                noseX++;
                            }
                            noseY++;
                        }
                        ...
                        for (int x = 4; x < 8; x++) {
                            for (int y = 0; y < 8; y++) {
                                noseTexture.setPixelColor(x, y, noseTexture.getPixelColor(7 - x, y));
                            }
                        }
                        //correct vertical alignment
                        for (int x = 0; x < 8; x++) {
                            for (int y = 0; y < 4; y++) {
                                int lowerColour = noseTexture.getPixelColor(x, y + 4);
                                noseTexture.setPixelColor(x, y + 4, noseTexture.getPixelColor(x, y));
                                noseTexture.setPixelColor(x, y, lowerColour);
                            }
                        }
