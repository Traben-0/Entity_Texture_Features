...
public class ETFManager {
    ...
    public  final ETFLruCache<UUID, ETFPlayerTexture> PLAYER_TEXTURE_MAP = new ETFLruCache<>(512);
    ...
    static final ETFLruCache<UUID, ImmutableList<String>> ENTITY_SPAWN_CONDITIONS_CACHE = new ETFLruCache<>();
    ...
    public <T extends Entity> ETFTexture getETFTexture(@NotNull Identifier vanillaIdentifier, @Nullable T entity, @NotNull TextureSource source, boolean canBePatched) {
                    ...
                    if (ENTITY_DEBUG_QUEUE.contains(id)) {
                        ...
                        ETFUtils2.logMessage(
                                "\nGeneral ETF:" +
                                        ...
                                        "\n - OptiFine property count: " + (OPTIFINE_PROPERTY_CACHE.containsKey(vanillaIdentifier) ? ETFUtils2.requireNonNullElse(OPTIFINE_PROPERTY_CACHE.get(vanillaIdentifier), new ArrayList<>()).size() : 0) +
        ...
        foundTexture = ETFUtils2.requireNonNullElse(getOrCreateETFTexture(vanillaIdentifier, possibleIdentifier == null ? vanillaIdentifier : possibleIdentifier, canBePatched), getETFDefaultTexture(vanillaIdentifier, canBePatched));
...
//                    @Nullable Boolean isAngryWithClient = null;
                    ...
                    if (props.containsKey("baby." + num)) {
                        ...
                        switch (dataFromProps) {
                            case "true" : baby = 1;break;
                            case "false" : baby = 2;break;
                        }
                    }
                    if (props.containsKey("weather." + num)) {
                        String dataFromProps = props.getProperty("weather." + num).trim();
                        switch (dataFromProps) {
                            case "clear" : weather = 1;break;
                            case "rain" : weather = 2;break;
                            case "thunder" : weather = 3;break;
                        }
                    }
